...
public class WindNimbus : ElectricNimbus {
        ...
        if (internalCounter >= 2) {
            ...
            foreach (RaycastHit hit in hitInfo) {
                if (hit.collider.gameObject.layer == Layers.Enemy) {
                    ...
                    hit.collider.GetComponent<Enemy>().ReactWind(Element.Types.Power, new Vector3(transform.position.x, hit.transform.position.y, transform.position.z));
                }
            }
            internalCounter = 0;
        }
